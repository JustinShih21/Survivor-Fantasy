...
export async function getPricesForEpisode(
  episode: number
): Promise<Record<string, number>> {
  const supabase = await createClient();
  const through = Math.max(1, episode);
  ...
  const priceByEpisode: Record<number, Record<string, number>> = {};
  ...
  for (let ep = 1; ep <= through; ep++) {
    ...
    const episodePrices: Record<string, number> = {};
    ...
    for (const c of contestants) {
      ...
      if (eliminated.has(cid)) {
        const prev = prevEpPrices?.[cid] ?? basePrice;
        episodePrices[cid] = prev;
        continue;
      }
      ...
      const prevPrice = prevEpPrices?.[cid] ?? basePrice;
      const perfRatio =
      ...
      episodePrices[cid] = rounded;
    }

    priceByEpisode[ep] = episodePrices;
  }

  return priceByEpisode[through] ?? basePrices;
}
