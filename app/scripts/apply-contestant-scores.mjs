 ...
 * Run from app directory: node scripts/apply-contestant-scores.mjs [path/to/scores.json]
 * Default path: ../scripts/data/contestant_trait_scores.json (repo scripts/data).
 */
...
function ensureEnv() {
  ...
  if (existsSync(fromApp)) loadEnvFrom(fromApp);
  else if (existsSync(fromRoot)) loadEnvFrom(fromRoot);
...
async function main() {
  const defaultPath = resolve(__dirname, "..", "..", "scripts", "data", "contestant_trait_scores.json");
